...
function Post({ post, active, modalPost,  setRemoveList,  len }) {
  ...
  useEffect(() => {
    getDoc(doc(db, "posts", user?.uid, 'userFavorites', post.id)).then((doc) => {
      if (doc.exists()) {
        setIsFavorite(true);
      } else {
        setIsFavorite(false);
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(query(collection(db, 'posts', post.id, 'comments'), orderBy('timestamp', 'desc'), limit(commentlen)), (snapshot) => {
      ...
      commentsAll.map((comment) => {
        const q = query(collection(db, 'posts', post.id, 'comments', comment.id, 'likes'), where('likes', 'array-contains', user?.uid));
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (!entry.isIntersecting) {
            if(active){
              getDoc(doc(db, 'posts', user?.uid, 'viewedPosts', post.id)).then((document) => {
                if(!document.exists()){
                  setDoc(doc(db, 'posts', user?.uid, 'viewedPosts', post.id), {
                    postID: post.id,
  ...
  const deletePost = async (post) => {
    if((post.type === "image" || post.type === "video") && post.share === false && post?.postURL){
      ...
      try {
        const q = query(collection(db, 'posts', user?.uid, "userPosts"), where("postID", "==", post.id));
        await onSnapshot(q, (snapshot) => {
          snapshot.docs.map(async (document) => {
            await deleteDoc(doc(db, 'posts', user?.uid, 'userPosts', document.id));
          })
        })
      } catch (error) {
        console.log(error);
  ...
  const loveComment = async (reply, comment) => {
    if(reply.likes.includes(user?.uid)){
      ...
      let index1 = comments[index].comments.map((comment) => comment.id).indexOf(reply.id);
      comments[index].comments[index1].likes = comments[index].comments[index1].likes.filter((like) => like !== user?.uid);
  ...
  const minimizeCountLength = (len) => {
    ...
    if(len >= 1000000 && len < 1000000000) return `${(len/1000000).toFixed(1)}M`;
    if(len >= 1000000000 && len < 1000000000000) return `${(len/1000000000).toFixed(1)}B`;
    if(len >= 1000000000000 && len < 1000000000000000) return `${(len/1000000000000).toFixed(1)}T`;
  }
  ...
  return (
    ...
    >
     ...
     {post.share ? (
         <div
         className="flex items-center px-2.5 cursor-pointer  "
         >
          <Link href={`/Profile?uid=${post?.shareFrom.uid}`} >
            <div>
             {post?.shareFrom?.photoURL && <img src={post?.shareFrom?.photoURL} className="!h-10 !w-10 cursor-pointer rounded-full object-cover" />}
             </div>
          </Link>
            <div className="mr-auto ml-2 leading-none">
              <Link href={`/Profile?uid=${post?.shareFrom?.uid}`} >
                <h6 className="font-medium hover:text-blue-500 hover:underline">
                  {post?.shareFrom?.name} <span className="text-xs dark:text-white/50 p-y-1 ">Shared</span>
                </h6>
              </Link>
              <p className="text-sm dark:text-white/50 p-y-1 ">{post?.shareFrom?.userName}</p>
              <p className="text-sm dark:text-white/50 p-y-1 ">{moment(post?.timestamp?.toDate()).fromNow()}</p>
            </div>
            {!isFavorite ? (
            <HeartIcon 
            ...
            onClick={() => {
              setIsFavorite(false)
              deleteDoc(doc(db, "posts", user?.uid, "userFavorites", post.shareFrom.id));
            }}
            className="h-6 w-6 text-red-500 cursor-pointer space-x-2 mr-2"
            />
              )}
        ...
        >
         <Link href={`/Profile?uid=${post?.user?.uid}`} >
           ...
           <div className="mr-auto ml-2 leading-none">
             ...
             <p className="text-sm dark:text-white/50 p-y-2 ">{post?.user?.userName}</p>
             <TimeAgo
           ...
           onClick={() => {
            setIsFavorite(false)
            deleteDoc(doc(db, "posts", user?.uid, "userFavorites", post.id));
           }}
           className="h-6 w-6 text-red-500 cursor-pointer space-x-2 mr-2"
           />
            )}
     ...
     {post?.share && post?.shareFrom.caption && (
        <div className="px-2.5 break-all md:break-normal" >
          {showInput ? (
            <p onClick={() => setShowInput(false)}>{ post?.shareFrom.caption}</p>
          ) : (
            <p onClick={() => setShowInput(true)}>
      ...
      <div className="flex items-center justify-between px-2.5">
        <div className="flex items-center space-x-2">
            ...
            onClick={() => {
              ...
              getDocs(collection(db, "posts", post.id, "likes")).then((snapshot) => {
                setModalUsers(snapshot.docs.map((doc) => ({id: doc.id, ...doc.data()})));
              });
            }}
            >
             {post?.likes > 0 ? `${liked ? 'You and ' : "" }${minimizeCountLength(post?.likes - liked)} others reacts •` : null}
            </p>
            <p   className="text-sm dark:text-white/50 hover:text-blue-500 hover:underline cursor-pointer"
             onClick={() => {
              setModalIsOpen(true);
        ...
        <div className="flex items-center space-x-2">
          <p className="text-sm dark:text-white/50 cursor-pointer
          hover:text-blue-500 hover:underline"
          onClick={() => {
            setModalIsOpen(true);
            setModalTitle("Shares");
            getDocs(collection(db, "posts", post.id, "shares")).then((snapshot) => {
              setModalUsers(snapshot.docs.map((doc) => ({id: doc.id, photoURL: doc.data().user.photoURL, name: doc?.data()?.user.name, userName: doc.data().user.userName})));
            });
          }}
          >{post?.shares > 0 ? `${minimizeCountLength(post.shares)} shares`: null}</p>
        </div>
      </div>
          ...
          {comments?.map((comment) => (
            <div key={comment.id} className="flex justify-start space-x-2">
              ...
              <div className="flex flex-col  ">
                ...
                <p className="text-sm dark:text-white/50 text-dark"
                 onClick={() => {
                 setShowComments(!showComments)
                }
                 }
                >
                  {showComments ?  comment?.comment :  truncate(comment?.comment, 100) }
                  </p>
                <div className="flex items-center space-x-2">
                  ...
                  >
                    {comment?.likes?.includes(user?.uid) ?<span className="text-blue-500
                    ">Loved</span> : "Love"}
                ...
                {comment.comments?.sort((a, b) => b.timestamp - a.timestamp).slice(0, len).map((reply) => (
                  ...
                  <div className="flex flex-col flex-grow ">
                    ...
                    <div className="flex items-center space-x-2 ">
                      ...
                      >
                        {reply?.likes?.includes(user?.uid) ?<span className="text-blue-500
                        ">Loved</span> : "Love"}
     ...
     {!showCommentInput && (
      ...
      >
        <div className="flex flex-col p-2">
          <div className="flex items-center justify-between">
            <h1 className="text-lg  font-semibold dark:text-white/75">{modalTitle}</h1>
            <button
      ...
      >
        <div className="flex flex-col p-2 ">
          <div className="flex items-center justify-between">
            <h1 className="text-lg  font-semibold dark:text-white/75">Share Post</h1>
            <button
              className="focus:outline-none"
