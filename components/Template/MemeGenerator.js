	...
	return (
     <>
        <div className='col-span-4'>
          <div className='upload w-full  text-center cursor-pointer bg-white text-gray-700 dark:bg-slate-800 p-3 dark:text-gray-400 dark:hover:text-white hover:text-gray-900 transition duration-200 hover:scale-x-105 mx-auto rounded-2xl mb-14'
                                ...
                                <div className='flex justify-center items-center'>
                                    <div className='w-1/2'>
                                        <ChipInput
                                            ...
                                            className="w-full bg-gray-200 dark:bg-slate-700 text-gray-400 dark:text-gray-400 rounded-sm p-3 placeholder:text-gray-400 placeholder:dark:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-opacity-50"
                                            value={tag}
                            ...
                            <div className='w-1/2'>
                                <button
                                    ...
                                    onClick={async () => {
                                        setLoading(true);
                                        if (meme) {
                                        const storageRef = ref(storage, `memes/${user.uid}/${meme.name}`);
                                        ...
                                        uploadTask.then(async () => {
                                            ...
                                            .then((ref) => {
                                                console.log("Added document with ID: ", ref.id);
                                                    addDoc(collection(db, "memes", user.uid, "userMemes"), {
                                                        memeURL: url,
                                                        ...
                                                        getDoc(doc(db, "memes", ref.id)).then((doc) => {
                                                            if (doc.exists()) {
                                                                ...
                                                                setMyMemes((prev) => [...prev, { ...doc.data(), id: doc.id }]);
                                                            } else {
