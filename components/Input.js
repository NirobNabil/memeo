...
function Input() {
  ...
  const textInput = useCallback((e) => {
    ...
    if(tagFriend.startsWith('@')) {
      ...
      getDocs(query(collection(db, 'users'), where('name', '>=', text), where('name', '<=', text + '\uf8ff'), orderBy('name', 'asc'), limit(5)))
      .then((querySnapshot) => {
        let friends = [];
        querySnapshot.forEach((doc) => {
          friends.push({id: doc.id, name: doc.data().name, photoURL: doc.data().photoURL, userName: doc.data()?.userName});
        });
  ...
  return (
    <div className="p-2 text-gray-500 font-medium border-b-2 border-gray-200 dark:border-gray-800 dark:bg-gray-900 rounded-sm">
        <div className="flex flex-col space-y-3">
          ...
          {loading && (
           <div className={`flex flex-col items-center space-y-2 bg-white dark:bg-slate-900 p-5 rounded-xl shadow-md w-full z-50 mt-5`}>
