...
function Home(props) {
  ...
  useEffect(() => {
    if(user) {
      setLoading(false)
      getDoc(doc(db, 'users', user?.uid, 'following', user?.uid)).then((doc) => {
        if(doc.exists()) {
  ...
  useEffect(() => {
    if (user) {
      getDoc(doc(db, 'users', user?.uid, 'followers', user?.uid)).then((Doc) => {
        if(Doc.exists()) {
    ...
    if (user) {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
          ...
          notifications?.map((notification) => {
            if(moment(notification?.notifidate?.toDate()).fromNow() === 'a few seconds ago') {
  ...
  useEffect(() => {
    if(notifications) {
      setNotificationsLength(notifications?.filter((notif) => !notif?.read).length);
    }
  }, [notifications])
  ...
  const fetchNotifications = () => {
    if (user && notifications?.length > 0) {
      ...
      const notificationsRef = collection(db, `notifications/${user?.uid}/notifications`);
      const q = query(notificationsRef, orderBy("notifidate", "desc"), startAfter(lastNotif?.notifidate), limit(10));
      const unsubscribe = onSnapshot(q, (snapshot) => {
          let notifications = snapshot.docs.map((doc) => {
              const data = doc.data();
              const id = doc.id;
    ...
    if (user && followingUIDs.length > 0) {
      followingUIDs.slice(0, 10).map((uid) => {
        ...
        getDoc(doc(db, 'users', uid)).then((doc) => {
          if(doc.exists()) {
            setFollowing((following) => [...following, {id: uid, ...doc.data()}]);
          }
        })
       }
      })
  ...
  const fetchFollowing = () => {
    if (user && followingUIDs.length > 0 && following?.length > 0) {
      ...
      followingUIDs.slice(followingUIDs.indexOf(lastFollowing?.id) + 1, followingUIDs.indexOf(lastFollowing?.id) + 11).map((uid) => {
        if(uid !== user?.uid) {
        getDoc(doc(db, 'users', uid)).then((doc) => {
          if(doc.exists()) {
            setFollowing((following) => [...following, {id: uid, ...doc.data()}]);
  ...
  useEffect(() => {
    if (user && followersUIDs.length > 0) {
      followersUIDs.slice(0, 10).map((uid) => {
        getDoc(doc(db, 'users', uid)).then((doc) => {
          if(doc.exists()) {
            setFollowers((followers) => [...followers, {id: uid, ...doc.data()}]);
          }
        })
      })
    }
  ...
  return (
    <>
    <div className="bg-[#F3F2EF] dark:bg-slate-900 dark:text-white h-screen">
      <Head>
        <title>Memex - The meme world</title>
      ...
      />
      <main className="min-h-screen flex  mx-auto  flex-col dark:bg-slate-900 dark:text-white bg-[#F3F2EF] text-black">
        <Suspense fallback={<Loading loading={true} />}>
          {tab === 'home' ? <Component {...props} 
          posts={posts} 
          active={true}
          following={following}
          follow={follow}
