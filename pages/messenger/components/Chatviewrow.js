...
function Users(props) {
	...
	useEffect(() => {
		const unsubscribe = onSnapshot(
			query(
				collection(db, "conversations", el.uid, "messages"),
	...
	function determineMsgtype(msg) {
		if (msg) {
			if (msg?.base64) {
				...
				return msg.message.replace(reg, (_, text, img) =>
					text ? `${text.trim()}` : `<Img src="${img}" />`
				);
			}
		}
	}
	...
	return (
		<div
			className='flex flex-row items-center justify-start w-full h-16 px-2 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-800 rounded-md'
			onClick={() => {
			...
			<div className='relative'>
				...
				/>
				<span
					className={`bottom-0 left-7 absolute z-50  w-3.5 h-3.5 ${
						reciponline ? "bg-green-400" : "bg-red-400"
					} border-2 border-white dark:border-gray-800 rounded-full`}></span>
			</div>
			<div
				className='flex flex-col ml-3 justify-between w-full'
				onClick={() => {
					setChatstarted(true);
				}}>
				<div className='flex flex-row justify-between'>
					...
					<p className='text-xs text-gray-500 truncate'>
						{msg?.msgdate?.toDate().toLocaleTimeString()}
					</p>
				</div>
				<div className='flex flex-row'>
					<p
						className='text-sm text-gray-500'
						dangerouslySetInnerHTML={{ __html: lastMsg(msg) }}></p>
				</div>
			</div>
		</div>
	);
