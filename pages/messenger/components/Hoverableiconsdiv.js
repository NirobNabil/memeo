...
function Hoverableicondiv(props) {
  ...
  const { inputref, progresswidth, progressbar,convoid, sendFile,special, icon,type, classNames, setState, state,  setState2, handleEvent } = props;
  ...
  return  (
    ...
    ?    /// theme change and other will added here
      <div 
      id='sth' onClick={()=>{chatFuncDisplay()}} >
    ...
    <label style={{zIndex: 4, cursor: 'pointer'}}>
        ...
        <>
         ...
         <Iconspecial  className={`${"icondiv fal fa-" + icon} bg-slate-50 dark:bg-slate-900`} />
         </>
        }
