			...
			msgs?.map((msg) => {
				if (msgidedit === msg.id) {
					if (msg?.senderid === user.uid) {
						updateDoc(
							doc(db, "conversations", convoid, "messages", msg.id),
							{
	...
	return (
		<div className='text-slate-500 dark:text-white'>
			<i
				...
				className={`${
					msg?.senderid === user?.uid
						? "fal fa-ellipsis-v-alt rightdots "
						: "fal fa-ellipsis-v-alt leftdots"
			...
			<div
				...
				className='flexrow optionsEdit bg-white dark:bg-slate-900 '
				ref={contextoption}
				data-id={msg?.msgid}
				style={{
