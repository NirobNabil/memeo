...
function Dialogue(props) {
  ...
  useEffect(() => {
    if(convoid){
      const unsubscribe = onSnapshot(
        ...
        (snapshot) => {
          setMessages(snapshot.docs.map((doc) => { return { ...doc.data(), id: doc.id } }
          ));
        }
      );
      return unsubscribe;
  ...
  useEffect(() => {
    if(convoid){
      const unsubscribe = onSnapshot(query(collection(db, "conversations", convoid, "messages"), orderBy("msgdate", "desc"), limit(100)), (querySnapshot) => {
  ...
  const appendMessages = (messages) => {
    if(messages?.id){
    const newMessages = messages.filter((message) => !messages.find((m) => m?.id === message?.id));
...
const showMessages =  messages?.sort((a, b) => b.msgdate - a.msgdate)?.map((msg, index) => {
   return ( 
           <div  key={index} id={"u" + msg.id} className={msg.senderid === user.uid ? "right m" : "left m"} onContextMenu={(e) => contextMenu(e)}>
              <div className="flex text-slate-500 dark:text-white">
                  ...
                  {msg.editing && <span className="text-xs text-gray-500">edited</span>}
                  </span>
                   <Threedots msgs={messages} user={user} msg={msg} convoid={convoid} />
                    <small className = {
                      msg.senderid === user?.uid
                        ? "alignRight elapsedtime"
                        : "alignLeft elapsedtime"
                    }>
                      <ReactTimeAgo date={msg.msgdate.toDate()} className='text-slate-500 dark:text-white/50' /> 
                  </small>
                  <div className="reaction"
                    ...
                    <i style={{marginLeft: msg.reaction2 === ''?'':'5px'}}className={ "emoj fas fa-" + msg.reaction2}></i>
                  </div>
                </div>
               { userimg && chatimg && <img src={msg.senderid === user.uid ? userimg : chatimg} alt="" /> }
              </div>
            );
         });
  ...
  const  sendFile = (type) => {
    ...
    if (file) {
        ...
        const childPath = `conversations/${convoid}/${user.uid}/${uuidv4()}.${extFile}`;
                    ...
                    if (extFile === "jpg" || extFile === "jpeg" || extFile === "png") {
                      ...
                      let notificationobj = {
                        ...
                        if(notifibool && chatuser){
                          addDoc(collection(db, "notifications", chatuser, "notifications"), notificationobj);
  ...
  useEffect(() => {
    ...
    getDoc(doc(db, 'users', chatuser)).then((doc) => {
      if (doc.exists()) {
        ...
        setChatimg(doc.data()?.photoURL || 'https://firebasestorage.googleapis.com/v0/b/instaclone-1c8c7.appspot.com/o/blank-profile-picture-973460_1280.png?alt=media&token=0b0f5b0f-0f5d-4b1f-8b1f-1b0f5d4b1f8b');
  ...
  const LoadMoreMessages = () => {
    setShowloadmore(false);
     getDocs(collection(db, 'conversations', convoid, 'messages')).then((docs) => {
      let messages = docs.docs.map((doc) => {
  ...
  return (
    <>
      ...
      className="top border-b borfer-gray-300 dark:border-gray-500 flex items-center justify-between"
      >
        {chatimg && (
        <>
        <div className="profilepic">
          <div className="relative">
              {chatimg && <img className="w-16 h-16 rounded-full" src={chatimg} alt=""/>}
              <span className={`bottom-0 left-6 absolute  w-3.5 h-3.5 ${chatactive ? "bg-green-400" : "bg-red-400"} border-2 border-white dark:border-gray-800 rounded-full`}></span>
           </div>
          <div>
            {nickname === '' ? 
            <p className='username'> 
            {chatname.length > 15 ? chatname.slice(0, 15) + 
            '...' : chatname 
            }
            </p> 
            : 
         ...
         <div className="controls text-slate-800">
          ...
          {/* {settings && (
            <div className="settings absolute top-0 right-0 bg-white dark:bg-gray-800 rounded-md shadow-md p-4">
              ...
              <div className="flex items-center justify-between mt-4">
                <p className="text-lg font-semibold">Start a Meeting</p>
                <i
                  className="fal fa-video text-xl cursor-pointer"
                  onClick={() => {
                    window.open(
                      `https://meet.jit.si/${chatname}`,
                      "_blank",
              ...
              <div className="flex items-center justify-between mt-4">
                ...
                <i
                  className="fal fa-phone-alt text-xl cursor-pointer"
                  onClick={() => {
                    window.open(`tel:${chatuser}`, '_blank')
                  }}
                ></i>
              </div>
            </div>
      ...
      >
        ...
        <div className="grid">
          ...
          <div className="scrollto chat" ref={scrollto}></div>
            ...
            onClick={()=> LoadMoreMessages()}>See More</button>}
        </div>
      </div>

   <div className="search whiteb flex h-20  items-center justify-between border-t border-gray-200 dark:border-gray-500 bg-white dark:bg-slate-800">
    ...
    <textarea
      ...
      className={`${type === 'small' ? "" : "focus:p-1"} placeholder:p-1 text-slate-600 placeholder:text-slate-600 mt-0.5`}

    ></textarea>
    <div className="gif_emoji  right-5 absolute space-x-2 top-4 ">
    <i className="fal fa-laugh" onClick={()=>{inputFunc()}}></i>
    <i className="fal fa-comment-alt-smile" onClick={()=>setGif(!gif)}></i>
    </div>
    ...
    ></i>
      ...
      <CSSTransition in={modal} classNames="modal" timeout={0}>
        <div className="modal overflowimgcontainer">
         ...
         <i className="fal fa-search-plus" onClick={()=>zoom({type: 'in'})}></i>
         </div>
      ...
      {/* <Modal
        isOpen={startVideoCall}
        onRequestClose={() => setStartVideoCall(false)}
        style={{
          overlay: {
      ...
      >
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="text-gray-500 text-sm">
            You can start a video call with {chatuser}
          </p>
      ...
      <Modal
        ...
        contentLabel="Example Modal"
      >
        <i className="fal fa-times p-2" onClick={() => setIsSettingModalOpen(false)}></i>

        <div className="flex flex-row items-center justify-between p-2">
          <h2 className="text-xl">Change Emoji</h2>
          <div className="flex flex-row items-center justify-between">
            <div className="flex flex-row items-center justify-between">
              <p className="text-gray-500 text-sm cursor-pointer" onClick={() => setEmojiTypePicker(!emojiTypePicker)}>
                {emojitype}
              </p>
            </div>
          </div>
        </div>
        {emojiTypePicker && <div className="flex flex-row items-center justify-between">
            <Picker
        ...
        <div className="flex flex-row items-center justify-between p-2">
          ...
          <div className="flex flex-row items-center justify-between">
            <div className="flex flex-row items-center justify-between">
               ...
               {theme ? 
               ( <img src={theme} alt="Background Image" className="w-16 h-16 rounded-sm cursor-pointer mr-4" onClick={() => document.getElementById("file").click()} />
               ) : (
                <div className="flex flex-row items-center justify-between">
                  <div className="w-16 h-16 rounded-sm cursor-pointer dark:bg-slate-800 bg-gray-200 mr-4" onClick={() => document.getElementById("file").click()}></div>
                </div>
               )}                  
              <button className="bg-blue-500 text-white px-4 py-2 rounded-md ml-2" onClick={() => {
                setTheme("");
                try {
                  const storageRef = ref(storage, `conversations/${convoid}/theme`);
              ...
              <input type="file" id="file" className="hidden" onChange={(e) => setTheme(URL.createObjectURL(e.target.files[0]))} />
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-row items-center justify-between p-2">
          ...
          <div className="flex flex-row items-center justify-between">
            <div className="flex flex-row items-center justify-between">
              <button className="bg-blue-500 text-white px-4 py-2 rounded-md" onClick={async () => {
